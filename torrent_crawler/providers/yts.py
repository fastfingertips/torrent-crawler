...
from torrent_crawler.utils.logger import log_runtime, logger
...
class YTSProvider(AbstractMovieProvider):
    ...
    def search(self, search_query: SearchQuery) -> list[Movie]:
        ...
        while has_next_page:
            ...
            if page_no > 1:
                request_url = f"{search_url}?page={page_no}"
    ...
    def get_details(self, movie: Movie) -> Movie:
        try:
            ...
            if movie_info:
                ...
                # Rating Info
                movie_ratings = movie_info.find("div", {"class": "bottom-info"}).find_all(
            ...
            if related_div:
                ...
                for a in related_div.find_all("a"):
                    ...
                    if title and link:
                        similar_movies.append({"title": title, "link": link, "image": img})
