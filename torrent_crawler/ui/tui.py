...
class MovieDetailScreen(Screen):
    ...
    def on_movie_detail_screen_subtitles_fetched(self, message: SubtitlesFetched) -> None:
        ...
        preferred_langs = [
            lang.lower() for lang in getattr(consts.settings, "user_preferences", {}).get("preferred_languages", [])
        ]

        # Sort subtitles: preferred languages first
        sorted_subtitles = sorted(message.subtitles.items(), key=lambda x: x[0].lower() not in preferred_langs)
